...
{
    // isIEntry palauttaa tiedon onko entryType jo k‰ytˆss‰. Palautusarvo on entry numero
...
void Person::assignNewPersoninDb()
{
    ...
    if (newId) {
        // aiemmin on luotu uusi henkilˆ, jota ei nyt k‰ytet‰k‰‰n. Tyhjennet‰‰n siis viimeisin tietokantaolento
...
void EntryLine::deleteThisPerson()
{
    ...
    // etsit‰‰n henkilˆ‰ joka pistet‰‰n current box:n
    ...
    // Koska t‰m‰ henkilˆ tuhottiin, suljetaan myˆs person-ikkuna ja annetaan return arvona
    // sen henkilˆn nimi joka pistet‰‰n currentiksi. Jos t‰m‰ henkilˆ oli current kun h‰net tuhottiin,
    // pit‰‰ siis currentiksi pist‰‰ joku toinen henkilˆ joka arvottiin tossa just ‰skett‰in.
...
// ------------------------- fams, famc, busb, wife data is ok, lets fill the persondata table --------------
    ...
    // annetaan palautusarvona henkilˆn id
...
void Person::closeWithoutSaving()
{
    ...
    // nuo *** merkityt oli k‰ytˆss‰, mutta tarvitaanko niit‰?
    //***for (quint8 b=0 ; b<visibleTabWidget->pEntryBox.size() ; b++) visibleTabWidget->pEntryBox.at(b)->deleteLater();
    // otetaan lisan pohjalla oleva "add" pois
    //***usrEvents.removeLast();
    //***famEvents.removeLast();
...
{    
    ...
    if (newType >= 0) { // t‰ss‰ oli ennest‰‰ jo joku eventti, jonka tyyppi muutetaan
        ...
        deleteUiWidgets();
        makeUi(entryBox->grid); // luoda uudelleen, koska uudessa event tyypiss‰ voi olla eri rivin sis‰ltˆ
...
void EntryLine::addEntry(int index)
{
    ...
    // myˆhemmin ainakin updatetype:ss‰, ja jos nyt valitaan jokin tieto johon ei liity entry‰, eli uiComboType:‰ ei m‰‰ritet‰
...
void EntryLine::slotUsrChanged(int newType)
{
    ...
    if (newType == 255) {
        ...
        // arvoon -1. *!* t‰t‰ voi varmaan soveltaa my√∂s event typess‰
...
void EntryLine::slotEraseEntryPhoto()
{
    ...
    // jos entryId on 255, k‰sitell‰‰n henkilˆn varsinaista kuvaa eik‰ siis entry:n (kuten kuolema) kuvaa
...
//## pit‰‰kˆ t‰ss‰ olla grid parametrin‰, eikˆ se ole aina boxin grid
{
    ...
    DE_BUG ele[i];
        ...
        if (ele[i] == DAY1 ) {
            ...
            if (   entryBox->entry->date1().isNull() && entryBox->entry->getAcc() != NA) {
                ...
                if (date.isNull())
                    // jos t‰m‰ on 1. henkilˆ, mit‰‰n arvausp‰iv‰m‰‰r‰‰ ei ole voitu laskea,
                    // annetaan t‰m‰ p‰iv‰
                    date = QDate::currentDate();

                // t‰ss‰ kohtaa pistet‰‰n laskettu p‰iv‰m‰‰r‰ehdotus my√∂s kantaan
...
void Person::nameEdited(QString name) {
    ...
    // t‰ss‰ kohtaa t‰m‰ voi aiheuttaa v‰‰ri‰ sexip‰‰telmi‰, esim jos pis‰‰ andersdotter, voi johtop‰‰tˆ olla ett‰ kyseess‰ on mies
...
Person::Person(bool sM, bool sF, quint16 *p1, QWidget *parent) : QDialog(parent)
{
    ...
    for (quint8 f=0 ; f<INDI(id).famSList.size() ; f++) {
        // onko puoliso todellakin olemassa? Jossain tilanteissa henkilˆll‰ voi olla spouse perhe-
        // numero, joka on siis annettu myˆs lapsen perhenumeroksi, mutta toista puolisoa ei olekaan
